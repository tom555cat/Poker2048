...
//        [self.layer setBackgroundColor:[UIColor whiteColor].CGColor];
        ...
        [APPDELEGATE.window addSubview:self];
         //新加
//        [APPDELEGATE.window sendSubviewToBack:self];
    }
    return self;
}
...
+ (void)showWithOverlay {
    [self dismiss:^{
        ...
        //新加
//         [APPDELEGATE.window sendSubviewToBack:[[self instance] overlay]];
        [self show];
    }];
}
...
- (UIView *)overlay {
    ...
    if (!self.overlayView) {
        ...
        [UIView animateWithDuration:FadeDuration animations:^{
            [self.overlayView setAlpha:0.05];
        }];
    }
