...
CGFloat const SDRefreshViewDefaultHeight = 110.0f;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
       ...
       // [self addSubview:stateIndicator];
        ...
        _loadingImageView=[[UIImageView alloc]initWithFrame:CGRectMake(0, 0,72,78)];
        NSMutableArray *array=[[NSMutableArray alloc]initWithCapacity:10];
        for (int i=0;i<9;i++) {
            UIImage *image=[UIImage imageNamed:[NSString stringWithFormat:@"lodingRe%d",i]];
            [array addObject:image];
            
        }
        _loadingImageView.image=[UIImage imageNamed:@"lodingRe0"];
        _loadingImageView.animationImages=array;
        _loadingImageView.animationDuration=0.6;
...
//    _timeIndicator.center = CGPointMake(self.sd_width * 0.5, self.sd_height - _timeIndicator.sd_height * 0.5 - SDTimeIndicatorMargin);
    ...
    _loadingImageView.center=CGPointMake(self.sd_width * 0.5, 35);
    ...
    switch (refreshState) {
        ...
        case SDRefreshViewStateRefreshing:
        {
            ...
            if (self.scrollView.contentOffset.y>-1) {
                _scrollView.contentInset = UIEdgeInsetsMake(self.frame.size.height/2+20,0,0,0);
            }
            else{
